-- Beancount transaction metadata supports user-defined scalar values. Store
-- each typed value relationally, while preserving legacy rows during upgrade.
DO $$
BEGIN
  IF to_regclass('ledger_index_transactions') IS NOT NULL THEN
    ...
    IF EXISTS (
      SELECT 1
      FROM ledger_index_transactions transactions
      CROSS JOIN LATERAL jsonb_each(transactions.metadata) AS metadata(key, value)
      WHERE jsonb_typeof(metadata.value) NOT IN ('null', 'string', 'number', 'boolean')
    ) THEN
      RAISE EXCEPTION 'ledger transaction metadata contains a non-scalar value';
    END IF;
    ...
    ON CONFLICT (revision_id, transaction_ordinal, metadata_key) DO NOTHING;

    ALTER TABLE ledger_index_transactions DROP COLUMN IF EXISTS metadata;
  END IF;
END $$;
