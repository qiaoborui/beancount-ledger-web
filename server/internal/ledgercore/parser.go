...
type MetadataValue = ledgermodel.MetadataValue
type BeanAmount = ledgermodel.BeanAmount
...
type BeanParseError = ParseError
...
type BeanParseResult = ParseResult
...
type beanToken = Token
type beanTokenKind = TokenKind

const (
	beanTokenWord   = TokenWord
	beanTokenString = TokenString
	beanTokenNumber = TokenNumber
	beanTokenTag    = TokenTag
	beanTokenLink   = TokenLink
	beanTokenPunct  = TokenPunct
)

func ParseLines(lines []Line) ParseResult {
	parser := beanLineParser{lines: lines}
	return parser.parse()
}

func ParseText(filename, text string) ParseResult {
	return ParseLines(beanTextLines(filename, text))
}

func CompileText(filename, text string) ParseResult {
	return CompileLines(beanTextLines(filename, text))
}

func beanTextLines(filename, text string) []Line {
	rawLines := strings.Split(text, "\n")
	lines := make([]Line, len(rawLines))
	for index, line := range rawLines {
		lines[index] = Line{File: filename, Line: index + 1, Text: strings.TrimSuffix(line, "\r")}
	}
	return lines
}

func CompileLines(lines []Line) ParseResult {
...
func ScanBeanLine(input string) []Token {
	return scanBeanLine(input)
}

func ParsePostingTokens(tokens []Token) (ParsedPosting, bool) {
	return parsePostingTokens(tokens)
}

func ParseMetadataLine(tokens []Token) (string, MetadataValue, bool) {
	return parseMetadataLine(tokens)
}

func ParseBeanAmountTokens(tokens []Token) (BeanAmount, int, bool) {
	return parseBeanAmountTokens(tokens)
}

func ParseCostTokenSpan(tokens []Token) (bool, int, bool) {
	return parseCostTokenSpan(tokens)
}

func DirectNumberText(tokens []Token) (string, bool) {
	return directNumberText(tokens)
}

func EvalNumberExpressionRat(tokens []Token) (*big.Rat, bool) {
	return evalNumberExpressionRat(tokens)
}

func IsIndentedBeanLine(text string) bool {
	return isIndentedBeanLine(text)
}

func IsTransactionFlag(value string) bool {
	return isTransactionFlag(value)
}

func IsPostingFlag(value string) bool {
	return isPostingFlag(value)
}

func IsBeanDateToken(value string) bool {
	return isBeanDateToken(value)
}

func IsBeanAccount(value string) bool {
	return isBeanAccount(value)
}

func IsBeanCurrency(value string) bool {
	return isBeanCurrency(value)
}
