...
export function MobileSheet({ open, title, children, footer, onClose, shouldClose, size = "lg", align = "right", bodyClassName = "", panelClassName = "", closeLabel = "关闭", zIndexClassName = "z-[100]" }: MobileSheetProps) {
  ...
  useFocusTrap({ open: mounted && open, containerRef: panelRef, initialFocusRef: closeButtonRef });
  ...
  return createPortal(
    <div className={`sheet-backdrop fixed inset-0 ${zIndexClassName} flex items-end bg-ink/35 p-0 ${desktopAlign}`} onClick={requestClose}>
      <div ref={panelRef} className={`mobile-sheet kami-float flex h-[92dvh] max-h-[calc(100dvh-env(safe-area-inset-top)-0.75rem)] w-full min-w-0 flex-col overflow-hidden rounded-t-[28px] bg-paper ${maxWidth} ${desktopRadius} ${panelClassName}`} style={{ transform: dragY > 0 ? `translate3d(0, ${dragY}px, 0)` : undefined }} onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
