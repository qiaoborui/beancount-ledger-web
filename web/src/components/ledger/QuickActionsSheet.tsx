...
export function QuickActionsSheet({ open, gitDirty, changedFileCount = 0, refreshing, onClose, onManualEntry, onAiEntry, onImport, onBalanceAssertion, onGitSave, onRefresh }: QuickActionsSheetProps) {
    ...
    <div className="grid gap-3">
      {actions.map((action) => {
        ...
        return <button key={action.label} type="button" disabled={action.disabled} onClick={() => run(action.onClick)} className={`flex items-center gap-3 rounded-2xl border border-line p-4 text-left disabled:opacity-50 ${action.primary ? "bg-brand text-paper" : "bg-panel text-warm"}`}>
          <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${action.primary ? "bg-paper/15 text-paper" : "bg-paper text-brand"}`}><Icon className={`h-5 w-5 ${action.label === "刷新中…" ? "animate-spin" : ""}`} /></span>
