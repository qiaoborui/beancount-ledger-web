...
function AssetComposition({ accounts, balances, visible }: { accounts: AccountView[]; balances: Record<string, number>; visible: boolean }) {
  ...
  return <section className="card p-4"><h2 className="font-serif text-2xl">当前资产明细</h2>{visible ? <div className="ledger-chart mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={rows} dataKey="value" nameKey="name" innerRadius={56} outerRadius={92} paddingAngle={2}>{rows.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value) => formatCny(Number(value))} /><Legend /></PieChart></ResponsiveContainer></div> : <HiddenMoney />}</section>;
}

function LiabilitiesTrend({ rows, visible }: { rows: ChartRow[]; visible: boolean }) {
  return <section className="card p-4"><h2 className="font-serif text-2xl">负债趋势</h2>{visible ? <div className="ledger-chart mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={rows} margin={{ left: 8, right: 16, top: 8, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" /><XAxis dataKey="date" minTickGap={18} /><YAxis width={56} tickFormatter={(value) => chartMoney(Number(value))} /><Tooltip formatter={(value) => formatCny(Number(value))} /><Area type="monotone" dataKey="负债" stroke="var(--chart-secondary)" fill="var(--chart-fill)" /></AreaChart></ResponsiveContainer></div> : <HiddenMoney />}</section>;
}
