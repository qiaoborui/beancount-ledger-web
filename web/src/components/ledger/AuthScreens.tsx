  ...
  return <section className="card p-6 text-center">
    ...
    {offline && offlineUnlockAvailable ? (
      <div className="mx-auto mt-5 flex max-w-sm flex-col gap-3">
        <Input type="password" className="h-12 rounded-xl bg-panel text-center" value={offlineSecret ?? ""} onChange={(event) => onOfflineSecretChange?.(event.target.value)} onKeyDown={(event) => event.key === "Enter" && onOfflineUnlock?.()} placeholder="离线解锁码" />
