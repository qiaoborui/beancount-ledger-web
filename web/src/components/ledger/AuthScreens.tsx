...
export function SensitiveUnlockPanel({ title = "资产信息已隐藏", description = "净资产和账户余额需要确认是你本人后查看。普通记账、流水、预算和损益分析可以直接使用。", message, onUnlock }: { title?: string; description?: string; message?: string; onUnlock: () => void }) {
  return <section className="card p-6 text-center"><div className="mx-auto mb-4 h-1 w-12 rounded-full bg-brand" /><h2 className="font-serif text-2xl font-medium">{title}</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-olive">{description}</p><button className="mt-5 rounded-xl bg-brand px-5 py-3 text-paper" onClick={onUnlock}>使用 Face ID / Passkey 查看</button>{message && <p className="mt-3 whitespace-pre-wrap text-sm text-[var(--danger)]">{message}</p>}<p className="mt-4 text-xs text-stone">解锁后 15 分钟内可查看余额和净资产；重新打开仍可先直接聊天。</p></section>;
}
