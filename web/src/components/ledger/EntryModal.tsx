...
export function EntryModal({ children, onClose }: { children: ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-40 grid place-items-end bg-ink/35 p-0 sm:place-items-center sm:p-4"><div className="kami-float max-h-[92vh] w-full overflow-y-auto rounded-t-2xl bg-paper p-4 sm:max-w-2xl sm:rounded-2xl"><div className="mb-3 flex items-center justify-between"><h2 className="font-serif text-2xl">记一笔</h2><button className="rounded-xl border border-line px-3 py-1 text-sm" onClick={onClose}>关闭</button></div>{children}</div></div>;
}
