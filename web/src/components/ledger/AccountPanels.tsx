...
export function CreditCardPanel({ cards, statuses, visible, onToggleAccount }: { cards: CreditCardAnalytics[]; statuses: AccountStatus[]; visibleAccountMap?: Record<string, boolean>; visible: boolean; onToggleAccount?: (account: string) => void }) {
  ...
  return <section className="card mt-6 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="font-serif text-2xl">信用卡视图</h2><p className="mt-1 text-sm text-olive">把信用卡从普通负债里单独拎出来，看当前未还、本期消费、还款和最近活动。</p></div><div className="rounded-2xl border border-line bg-panel px-4 py-3 text-right"><div className="text-xs text-stone">总未还</div><div className="amount-expense text-xl font-semibold">{mask(formatCny(totalOutstanding / 100))}</div></div></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><CreditSummary label="本期刷卡" value={mask(formatCny(totalSpend / 100))} /><CreditSummary label="本期还款" value={mask(formatCny(totalRepayments / 100))} /><CreditSummary label="卡片数量" value={`${cards.length}`} /></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{cards.map((card) => { const st = statuses.find((item) => item.account === card.account); return <div key={card.account} className="rounded-2xl border border-line bg-panel p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 text-sm font-medium text-olive">{st && <span className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${statusColor(st.status)}`} title={statusTitle(st)} />}{card.label}</div><div className="mt-1 truncate text-xs text-stone">{card.account}</div></div>{onToggleAccount && <button className="shrink-0 rounded-xl border border-line px-2 py-1 text-olive hover:bg-tag" onClick={() => onToggleAccount(card.account)} title="切换该账户余额显示">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>}</div><div className="amount-expense mt-3 text-2xl font-semibold">{mask(formatCompactCny(card.outstanding / 100))}</div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><CreditSummary label="本期消费" value={mask(formatCny(card.periodSpend / 100))} /><CreditSummary label="本期还款" value={mask(formatCny(card.periodRepayments / 100))} /><CreditSummary label="消费笔数" value={`${card.txCount}`} /><CreditSummary label="最近活动" value={card.lastActivityDate?.slice(5) ?? "—"} /></div></div>; })}</div></section>;
}
