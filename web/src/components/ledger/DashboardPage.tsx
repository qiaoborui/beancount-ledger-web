...
export function DashboardPage({ timeRange, valuationCurrency, visible, onToggleVisible, onSensitiveLocked, onOpenTransactions }: { timeRange: TimeRange; valuationCurrency: string; visible: boolean; onToggleVisible: () => void; onSensitiveLocked: () => void; onOpenTransactions: (href: string) => void }) {
  ...
  if (loading && !data) return <DashboardStatusCard title={t("dashboard.loadingTitle")} detail={t("dashboard.loadingDetail")} icon={<RefreshCw className="h-4 w-4 animate-spin text-brand" />} />;
  if (error && !data) return <DashboardStatusCard title={t("dashboard.loadFailed")} detail={error} icon={<AlertTriangle className="h-4 w-4 amount-danger" />} actionLabel={t("dashboard.retry")} onAction={reload} />;
  if (!data) return <DashboardStatusCard title={t("dashboard.noData")} detail={t("dashboard.noDataDetail")} actionLabel={t("dashboard.reload")} onAction={reload} />;
  ...
  const topCategoryText = topCategory ? `${topCategory.label} · ${mask(compact(topCategory.total / 100))}` : t("dashboard.noDataShort");
  ...
  const panels: Record<DashboardPanelId, DashboardPanelDefinition> = {
    dailyExpense: {
      title: t("dashboard.dailyExpenseTitle"),
      subtitle: t("dashboard.dailyExpenseSubtitle", { count: data.dailyExpenseSeries.length }),
      render: () => visible ? <DailyExpenseChart data={data} onOpenTransactions={onOpenTransactions} /> : <HiddenChart />,
    },
    weekdayExpense: {
      title: t("dashboard.weekdayTitle"),
      subtitle: t("dashboard.weekdaySubtitle"),
      render: () => visible ? <WeekdayExpenseChart data={data} /> : <HiddenChart />,
    },
    categoryRank: {
      title: t("dashboard.categoryRankTitle"),
      subtitle: t("dashboard.categoryRankSubtitle", { count: data.categorySeries.length }),
    ...
    payeeRank: {
      title: t("dashboard.payeeRankTitle"),
      subtitle: t("dashboard.payeeRankSubtitle", { count: data.topPayees.length }),
      render: () => <PayeeList data={data} visible={visible} onOpenTransactions={onOpenTransactions} />,
    },
    paymentAccounts: {
      title: t("dashboard.paymentAccountsTitle"),
      subtitle: t("dashboard.paymentAccountsSubtitle", { count: data.topPaymentAccounts.length }),
      render: () => <PaymentAccounts data={data} visible={visible} onOpenTransactions={onOpenTransactions} />,
    },
    anomalies: {
      title: t("dashboard.anomaliesTitle"),
      subtitle: t("dashboard.anomaliesSubtitle", { count: data.anomalies.length }),
    ...
    categoryTrend: {
      ...
      subtitle: t("dashboard.categoryTrendSubtitle", { count: data.categorySeries.length }),
  ...
  return <div className="dashboard-workbench">
    ...
    {loading && <DashboardNotice tone="loading" title={t("dashboard.refreshTitle")} detail={t("dashboard.refreshDetail")} />}
    {error && <DashboardNotice tone="error" title={t("dashboard.refreshFailed")} detail={error} actionLabel={t("dashboard.retry")} onAction={reload} />}
    {dashboardEmpty ? <DashboardEmptyState filtered={activeFilters} onClearFilters={clearFilters} onRetry={reload} /> : <>

    <DashboardInlineRow rowId="monitor" title={t("dashboard.monitorTitle")} subtitle={t("dashboard.monitorSubtitle")} collapsed={collapsedRows.monitor} onToggle={toggleRow} summary={<RowSummary>{t("dashboard.monitorSummary", { value: mask(compact(data.kpis.expense / 100)), days: activeExpenseDays })}</RowSummary>}>
      <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div className="grid flex-1 grid-cols-2 divide-x divide-y divide-line overflow-hidden border-y border-line sm:grid-cols-3 xl:grid-cols-5 xl:divide-y-0">
          <Kpi label={t("dashboard.filteredExpense")} value={mask(compact(data.kpis.expense / 100))} tone="text-warm" />
          <Kpi label={t("dashboard.activeDailyAverage")} value={mask(compact(averageExpense / 100))} tone="text-warm" />
          <Kpi label={t("dashboard.activeDays")} value={t("dashboard.daysSuffix", { count: activeExpenseDays })} tone="text-warm" />
          <Kpi label={t("dashboard.largestSingle")} value={mask(compact(maxExpense / 100))} tone="text-warm" />
          <Kpi label={t("dashboard.largeExpenses")} value={t("dashboard.countSuffix", { count: data.anomalies.length })} tone={data.anomalies.length ? "amount-danger" : "text-warm"} />
        </div>
        <button className="h-10 shrink-0 self-end rounded-md border border-line bg-panel px-2.5 text-sm text-olive hover:bg-tag md:h-8 lg:self-auto" onClick={onToggleVisible} aria-label={visible ? t("dashboard.hideAnalysisAmounts") : t("dashboard.showAnalysisAmounts")} title={visible ? t("dashboard.hideAnalysisAmounts") : t("dashboard.showAnalysisAmounts")}>
          {visible ? <EyeOff className="inline h-4 w-4 text-brand" /> : <Eye className="inline h-4 w-4 text-brand" />} <span className="ml-1">{visible ? t("dashboard.hideAmounts") : t("dashboard.showAmounts")}</span>
        </button>
      </div>
    </DashboardInlineRow>

    <DashboardRow rowId="spending" title={t("dashboard.spendingTitle")} subtitle={t("dashboard.spendingSubtitle")} collapsed={collapsedRows.spending} onToggle={toggleRow} summary={<RowSummary>{t("dashboard.spendingSummary", { days: data.dailyExpenseSeries.length, buckets: data.weekdayExpense.length })}</RowSummary>}>
    ...
    <DashboardRow rowId="risk" title={t("dashboard.riskTitle")} subtitle={t("dashboard.riskSubtitle")} collapsed={collapsedRows.risk} onToggle={toggleRow} summary={<RowSummary>{t("dashboard.riskSummary", { top: topCategoryText, count: data.anomalies.length })}</RowSummary>}>
    <div className="dashboard-panel-grid">
...
async function fetchDashboardSummary(params: string, cacheKey: string) {
  ...
  const request = (async () => {
    ...
    if (!response.ok) throw new Error(next.error || i18n.t("dashboard.requestFailed", { status: response.status }));
    return next;
  })();
...
function DashboardEmptyState({ filtered, onClearFilters, onRetry }: { filtered: boolean; onClearFilters: () => void; onRetry: () => void }) {
  ...
  return <section className="border-b border-line bg-panel p-5">
    <div className="mx-auto max-w-xl text-center">
      <h2 className="text-xl font-semibold tracking-[-0.018em] text-warm">{filtered ? t("dashboard.emptyFilteredTitle") : t("dashboard.emptyNoDataTitle")}</h2>
      <p className="mt-2 text-sm text-stone">
        {filtered ? t("dashboard.emptyFilteredDetail") : t("dashboard.emptyNoDataDetail")}
      </p>
      <div className="mt-4 flex flex-wrap justify-center gap-2">
        {filtered && <button type="button" className="inline-flex h-9 items-center justify-center rounded-lg border border-line bg-panel px-3 text-sm text-olive hover:bg-tag" onClick={onClearFilters}>{t("dashboard.clearFilters")}</button>}
  ...
  return <section className="border-b border-line bg-panel px-3 py-3 transition-colors md:px-4">
    <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
      <label className="min-w-0 flex-1">
        <span className="mb-1 block text-[11px] text-stone">{t("dashboard.queryLabel")}</span>
        <Input className="h-10 w-full min-w-0 rounded-md bg-panel text-sm text-olive md:h-9" placeholder={t("dashboard.queryPlaceholder")} value={queryDraft} list={queryRecentsListId} onChange={(event) => setQueryDraft(event.target.value)} />
      ...
      <button type="button" className="flex h-9 shrink-0 items-center justify-center gap-2 rounded-md border border-line bg-panel px-3 text-left text-sm font-medium text-warm hover:bg-tag hover:text-brand" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
        ...
        <span className="min-w-0 truncate">{t("dashboard.advancedFilter")}</span>
        <span className="shrink-0 rounded-full bg-tag px-2 py-0.5 text-xs font-normal text-stone">{advancedChips.length ? t("dashboard.conditionsCount", { count: advancedChips.length }) : t("dashboard.notEnabled")}</span>
    ...
    {!expanded && advancedChips.length > 0 && <div className="mt-3 flex min-w-0 flex-wrap items-center gap-2">
      ...
      <button type="button" className="rounded-full border border-line px-2.5 py-1 text-xs text-stone hover:bg-tag" onClick={clearAdvancedFilters}>{t("dashboard.clearAdvanced")}</button>
    </div>}
    {expanded && <>
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        <MultiFilterSelect label={t("dashboard.categoryLabel")} value={filters.category} onChange={(value) => onChange("category", value)} options={data.filterOptions.categories} />
        <MultiFilterSelect label={t("dashboard.accountLabel")} value={filters.account} onChange={(value) => onChange("account", value)} options={data.filterOptions.accounts} />
        <MultiFilterSelect label={t("dashboard.payeeLabel")} value={filters.payee} onChange={(value) => onChange("payee", value)} options={data.filterOptions.payees} />
        <MultiFilterSelect label={t("dashboard.tagLabel")} value={filters.tag} onChange={(value) => onChange("tag", value)} options={data.filterOptions.tags} />
        <MoneyFilterInput label={t("dashboard.minAmountLabel")} value={minAmountDraft} onChange={setMinAmountDraft} />
        <MoneyFilterInput label={t("dashboard.maxAmountLabel")} value={maxAmountDraft} onChange={setMaxAmountDraft} />
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {chips.length ? chips.map((chip) => <FilterChip key={chip.key} chip={chip} onClear={onClear} />) : <span className="text-xs text-stone">{t("dashboard.noVariableFilter")}</span>}
        {chips.length > 0 && <button type="button" className="rounded-full border border-line px-2.5 py-1 text-xs text-stone hover:bg-tag" onClick={onClearAll}>{t("dashboard.clear")}</button>}
      </div>
    </>}
  </section>;
}
...
function FilterChip({ chip, onClear }: { chip: { key: DashboardFilterKey; label: string }; onClear: (key: DashboardFilterKey) => void }) {
  const { t } = useTranslation();
  return <button type="button" className="inline-flex max-w-full items-center gap-1 rounded-full border border-line bg-tag px-2.5 py-1 text-xs text-olive hover:bg-panel" onClick={() => onClear(chip.key)} title={t("dashboard.removeFilter")}>
  ...
  return <div className="min-w-0">
    ...
    <details className="group relative">
      <summary className="flex min-h-10 cursor-pointer list-none items-center justify-between gap-2 rounded-md border border-line bg-panel px-2 py-2 text-sm text-olive outline-none group-open:border-brand md:min-h-8 md:py-1.5">
        <span className="min-w-0 truncate">{value.length ? t("dashboard.itemsCount", { count: value.length }) : t("dashboard.all")}</span>
      ...
      <div className="absolute inset-x-0 z-30 mt-1 max-h-72 overflow-auto rounded-md border border-line bg-paper p-2 shadow-lg">
        ...
        }) : <div className="px-2 py-3 text-sm text-stone">{t("dashboard.noOptions")}</div>}
      </div>
    </details>
  </div>;
}
...
function MoneyFilterInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  return <label className="min-w-0">
    <span className="mb-1 block text-[11px] text-stone">{label}</span>
    <Input className="h-10 w-full min-w-0 rounded-md bg-panel text-sm text-olive md:h-8" inputMode="decimal" placeholder={t("dashboard.all")} value={value} onChange={(event) => onChange(event.target.value)} />
  </label>;
}
...
function activeFilterChips(data: DashboardSummary, filters: DashboardFilterState) {
  ...
  if (filters.category.length) chips.push({ key: "category", label: `${i18n.t("dashboard.categoryPrefix", { value: filters.category.map((value) => optionLabel(data.filterOptions.categories, value)).join(" / ") })}` });
  add("query", i18n.t("dashboard.queryLabel"), filters.query);
  if (filters.account.length) chips.push({ key: "account", label: `${i18n.t("dashboard.accountPrefix", { value: filters.account.map((value) => optionLabel(data.filterOptions.accounts, value)).join(" / ") })}` });
  if (filters.payee.length) chips.push({ key: "payee", label: `${i18n.t("dashboard.payeePrefix", { value: filters.payee.join(" / ") })}` });
  if (filters.tag.length) chips.push({ key: "tag", label: `${i18n.t("dashboard.tagPrefix", { value: filters.tag.join(" / ") })}` });
  add("minAmount", i18n.t("dashboard.minPrefix"), filters.minAmount);
  add("maxAmount", i18n.t("dashboard.maxPrefix"), filters.maxAmount);
  return chips;
}
...
function DashboardPanelView({ panel, onClose }: { panel: DashboardPanelDefinition; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[130] bg-[rgba(20,20,19,0.72)] p-3 backdrop-blur-sm sm:p-5" role="dialog" aria-modal="true" aria-label={i18n.t("dashboard.fullscreenView", { title: panel.title })} onClick={onClose}>
    <section className="dashboard-panel-view card mx-auto flex h-[calc(100dvh-1.5rem)] max-w-7xl flex-col p-4 sm:h-[calc(100dvh-2.5rem)] sm:p-5" style={viewStyle} onClick={(event) => event.stopPropagation()}>
      <div className="flex shrink-0 items-start justify-between gap-3 border-b border-line pb-3">
        ...
        <button type="button" className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-line bg-panel text-stone hover:bg-tag hover:text-brand" onClick={onClose} title={i18n.t("dashboard.close")} aria-label={i18n.t("dashboard.closeFullscreen")}>
          <X className="h-5 w-5" />
        </button>
      </div>
  ...
  return <section className={`dashboard-panel-shell min-w-0 border-b border-line bg-panel ${className ?? ""}`}>
    <div className="flex min-h-11 items-center justify-between gap-3 border-b border-line px-3 py-2 md:px-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button type="button" className="grid h-6 w-6 place-items-center rounded-md text-stone hover:bg-tag hover:text-ink" onClick={() => onView(panelId)} title={i18n.t("dashboard.fullscreenLabel")} aria-label={i18n.t("dashboard.fullscreenView", { title })}>
    ...
    <ChartBox empty={!rows.length}>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={rows} margin={{ left: 8, right: 16, top: 14, bottom: 0 }} barCategoryGap="30%" onClick={(state) => {
          ...
          <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => name === "笔数" ? [Number(value), i18n.t("dashboard.countSeries")] : [formatValuation(Number(value), data.currency), i18n.t("dashboard.expenseSeries")]} />
...
export function InteractiveLegend({ series, focusedAccount, onToggle, expandOnWideScreens = false }: { series: { account: string; alias?: string | null; label: string }[]; focusedAccount: string | null; onToggle: (account: string) => void; expandOnWideScreens?: boolean }) {
  ...
  return <div className={`mt-2 flex max-h-20 flex-wrap items-center justify-center gap-x-3 gap-y-2 overflow-y-auto px-1 text-xs ${expandOnWideScreens ? "sm:max-h-none sm:overflow-visible" : ""}`} aria-label={i18n.t("dashboard.legendLabel")}>
...
function CategoryRank({ rows, currency, visible, onOpenTransactions }: { rows: DashboardSummary["categorySeries"]; currency: string; visible: boolean; onOpenTransactions: (href: string) => void }) {
  if (!rows.length) return <EmptyPanel text={i18n.t("dashboard.noCategoryExpense")} />;
...
function PayeeList({ data, visible, onOpenTransactions }: { data: DashboardSummary; visible: boolean; onOpenTransactions: (href: string) => void }) {
  if (!data.topPayees.length) return <EmptyPanel text={i18n.t("dashboard.noPayeeData")} />;
  ...
  return <div className="mt-3 space-y-2.5">
    {data.topPayees.slice(0, 8).map((row) => <button key={row.payee} className="w-full text-left" onClick={() => onOpenTransactions(transactionHref({ q: row.payee }))}>
      <ResponsiveValueRow label={row.payee} labelClassName="truncate text-sm text-olive" value={visible ? formatCompactValuation(row.amount / 100, data.currency) : "••••••"} valueClassName="text-sm font-semibold text-warm" valueTitle={visible ? formatCompactValuation(row.amount / 100, data.currency) : i18n.t("dashboard.amountHidden")} />
      <div className="mt-1 flex items-center gap-2">
        ...
        <span className="w-10 text-right text-xs text-stone">{i18n.t("dashboard.txCountShort", { count: row.txCount })}</span>
      </div>
    </button>)}
  </div>;
}
...
function AnomalyList({ rows, currency, visible, onSelectCategory }: { rows: DashboardSummary["anomalies"]; currency: string; visible: boolean; onSelectCategory: (account: string, mode?: "exact" | "prefix") => void }) {
  if (!rows.length) return <EmptyPanel text={i18n.t("dashboard.noLargeExpenses")} />;
...
function PaymentAccounts({ data, visible, onOpenTransactions }: { data: DashboardSummary; visible: boolean; onOpenTransactions: (href: string) => void }) {
  if (!data.topPaymentAccounts.length) return <EmptyPanel text={i18n.t("dashboard.noPaymentAccounts")} />;
...
function ChartBox({ empty, compact = false, children }: { empty: boolean; compact?: boolean; children: ReactNode }) {
  if (empty) return <EmptyPanel text={i18n.t("dashboard.noTrendData")} compact={compact} />;
...
function HiddenChart({ compact = false }: { compact?: boolean }) {
  return <div className={`dashboard-chart-canvas mt-3 grid place-items-center border border-line bg-panel text-sm text-stone ${compact ? "dashboard-chart-canvas-compact" : ""}`}>{i18n.t("dashboard.amountHidden")}</div>;
}
