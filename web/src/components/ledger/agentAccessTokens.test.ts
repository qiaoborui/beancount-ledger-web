...
describe("Agent access token API", () => {
  ...
  it("creates a token without the removed write-scope field", async () => {
    const fetchMock = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => new Response(JSON.stringify({ token: "token", credential: { id: "1", name: "writer", createdAt: "2026-08-05T00:00:00Z", expiresAt: "2026-11-03T00:00:00Z" } }), { status: 201 }));
    vi.stubGlobal("fetch", fetchMock);

    await createAgentAccessToken("writer");

    expect(JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body))).toEqual({ name: "writer" });
  });
