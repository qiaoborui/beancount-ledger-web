...
export function CashflowTrendChart({ rows, currency, cumulative = false }: { rows: DashboardCashflowPoint[]; currency: string; cumulative?: boolean }) {
  ...
  if (!rows.length) return <ChartEmpty text={t("homeCharts.noCashflow")} />;
  let income = 0;
  let expense = 0;
  const data = rows.map((row) => {
    ...
    return {
      label: row.month,
      income: (cumulative ? income : row.income) / 100,
      expense: (cumulative ? expense : row.expense) / 100,
      net: (cumulative ? income - expense : row.net) / 100,
    };
  });
  const seriesNames: Record<string, string> = {
    income: t("homeCharts.income"),
    expense: t("homeCharts.expense"),
    net: t("homeCharts.netIncome"),
  };
  return <ChartFrame>
    <LineChart data={data} margin={{ top: 14, right: 16, bottom: 2, left: 4 }}>
      ...
      <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [formatValuation(Number(value), currency), seriesNames[String(name)] ?? String(name)]} />
      <Line type="linear" dataKey="income" stroke="var(--ink)" strokeWidth={1.5} dot={false} activeDot={{ r: 3, strokeWidth: 0 }} />
      <Line type="linear" dataKey="expense" stroke="var(--stone)" strokeWidth={1.35} strokeDasharray="5 4" dot={false} activeDot={{ r: 3, strokeWidth: 0 }} />
      <Line type="linear" dataKey="net" stroke="var(--chart-primary)" strokeWidth={1.75} dot={false} activeDot={{ r: 3, strokeWidth: 0 }} />
    </LineChart>
  </ChartFrame>;
}
...
export function PeriodComparisonChart({ current, previous, metric, currency, currentLabel, previousLabel }: { current: DashboardCashflowPoint[]; previous: DashboardCashflowPoint[]; metric: ComparisonMetric; currency: string; currentLabel: string; previousLabel: string }) {
  ...
  if (!data.length) return <ChartEmpty text={t("homeCharts.noYoYTrend")} />;
  return <ChartFrame>
...
export function CategoryDistributionChart({ series, currency }: { series: DashboardCategorySeries[]; currency: string }) {
  ...
  if (!data.length) return <ChartEmpty text={t("homeCharts.noCategory")} />;
  return <div className="relative h-full min-h-0">
    ...
    <div className="pointer-events-none absolute inset-0 grid place-items-center text-center">
      <div><div className="text-[11px] text-stone">{t("homeCharts.categoryExpense")}</div><div className="mt-1 text-lg font-semibold tabular-nums text-ink">{formatCompactValuation(total, currency)}</div></div>
    </div>
  </div>;
}
...
export function CategoryComparisonChart({ current, previous, account, currency, currentLabel, previousLabel }: { current: DashboardCategorySeries[]; previous: DashboardCategorySeries[]; account: string; currency: string; currentLabel: string; previousLabel: string }) {
  ...
  if (!data.length) return <ChartEmpty text={t("homeCharts.noCategoryTrend")} />;
  return <ChartFrame>
    <LineChart data={data} margin={{ top: 14, right: 16, bottom: 2, left: 4 }}>
      <CartesianGrid stroke="var(--chart-grid)" strokeOpacity={0.78} vertical={false} />
      <XAxis dataKey="label" tick={{ fill: "var(--stone)", fontSize: 11 }} tickLine={false} axisLine={{ stroke: "var(--line)" }} minTickGap={16} />
...
export function PaymentAccountChart({ rows, currency }: { rows: AccountAnalytics[]; currency: string }) {
  const { t } = useTranslation();
  const data = rows.slice(0, 6).map((row) => ({ label: row.label || row.account, expense: row.amount / 100 }));
  if (!data.length) return <ChartEmpty text={t("homeCharts.noPaymentAccounts")} />;
  return <ChartFrame>
    <BarChart data={data} margin={{ top: 14, right: 16, bottom: 8, left: 4 }}>
      ...
      <YAxis width={56} tick={{ fill: "var(--stone)", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={(value) => formatCompactValuation(Number(value), currency)} />
      <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatValuation(Number(value), currency), t("homeCharts.expense")]} />
      <Bar dataKey="expense" fill="var(--stone)" radius={[2, 2, 0, 0]} maxBarSize={54} />
    </BarChart>
  </ChartFrame>;
}
...
export function AccountBalanceTrendChart({ series, account, currency }: { series: DashboardAccountSeries[]; account: string; currency: string }) {
  ...
  const data = selected?.values.map((row) => ({ label: row.month, balance: row.value / 100 })) ?? [];
  if (!data.length) return <ChartEmpty text={t("homeCharts.noBalanceTrend")} />;
  return <ChartFrame>
    <AreaChart data={data} margin={{ top: 14, right: 16, bottom: 2, left: 4 }}>
      ...
      <YAxis width={56} tick={{ fill: "var(--stone)", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={(value) => formatCompactValuation(Number(value), currency)} />
      <ReferenceLine y={0} stroke="var(--line-soft)" />
      <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatValuation(Number(value), currency), t("homeCharts.balance")]} />
      <Area type="linear" dataKey="balance" stroke="var(--ink)" strokeWidth={1.6} fill="url(#home-account-fill)" dot={false} activeDot={{ r: 3, strokeWidth: 0 }} />
    </AreaChart>
  </ChartFrame>;
}
...
export function ExpenseHeatmap({ rows, start, end, currency }: { rows: DashboardDailyExpense[]; start: string; end: string; currency: string }) {
  ...
  if (!months.length) return <ChartEmpty text={t("homeCharts.noDates")} />;
  return <div className="overflow-x-auto pb-1">
    <div className="grid min-w-[62rem] grid-cols-12 gap-3">
      {months.map((month) => <div key={month} className="min-w-0">
        <div className="mb-2 text-center text-[11px] font-medium tabular-nums text-stone">{Number(month.slice(5))}{t("homeCharts.monthSuffix")}</div>
        <div className="grid grid-cols-7 gap-1">
          {monthCells(month).map((date, index) => date ? <span key={date} className={`aspect-square min-w-0 rounded-[2px] ${heatClass(amountByDate.get(date) ?? 0, max)}`} title={`${date} · ${formatValuation((amountByDate.get(date) ?? 0) / 100, currency)}`} aria-label={`${date} ${t("homeCharts.expense")} ${formatValuation((amountByDate.get(date) ?? 0) / 100, currency)}`} /> : <span key={`${month}-${index}`} className="aspect-square" />)}
        </div>
      </div>)}
    </div>
    <div className="mt-4 flex items-center justify-end gap-1.5 text-[10px] text-stone"><span>{t("homeCharts.low")}</span><span className="h-2.5 w-5 rounded-[2px] bg-tag" /><span className="h-2.5 w-5 rounded-[2px] bg-brand/25" /><span className="h-2.5 w-5 rounded-[2px] bg-brand/55" /><span className="h-2.5 w-5 rounded-[2px] bg-brand" /><span>{t("homeCharts.high")}</span></div>
  </div>;
}
