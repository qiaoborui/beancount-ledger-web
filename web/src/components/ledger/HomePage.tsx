...
export function HomePage({ summary, chart, privacySettings, sensitiveUnlocked, onPrivacyChange }: { summary: Summary | null; chart: { day: string; 收入: number; 支出: number }[]; privacySettings: PrivacySettings; sensitiveUnlocked: boolean; onPrivacyChange: <K extends keyof PrivacySettings>(key: K, value: PrivacySettings[K]) => void }) {
  return <>
    <section className="card overflow-hidden p-0">
      <div className="border-l-4 border-brand p-5 md:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-olive">首页只放月度方向和最近流水；收入、结余、余额和净资产需要解锁后查看。</p>
          </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-line border-t border-line p-5 text-center">
        <Metric label="收入" value={privacySettings.showHomeSummaryAmounts && sensitiveUnlocked ? formatCny((summary?.income ?? 0) / 100) : "••••••"} cls="amount-income text-lg sm:text-2xl" />
        ...
        <Metric label="结余" value={privacySettings.showHomeSummaryAmounts && sensitiveUnlocked ? formatCny((summary?.net ?? 0) / 100) : "••••••"} cls="amount-gold text-lg sm:text-2xl" />
      </div>
    </section>
