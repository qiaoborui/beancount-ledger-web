...
- `Detail(ctx, id)`: exact directive/posting/metadata records capped at 1 MiB. Oversized detail returns an explicit resource-limit error; it does not truncate. Chunked detail is a future capability.
