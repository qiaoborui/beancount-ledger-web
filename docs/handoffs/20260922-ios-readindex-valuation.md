...
- Status: active; updated 2026-09-22T15:13:00+08:00. Real simulator app-container publication now passes; explicit BoundedRelease packaging implemented. Physical-device preparation blocked the first test attempt; broader device acceptance remains open.
...
- Next: publish this milestone/check CI; build and verify BoundedRelease IPA with separate LAN delivery; retry device tests after trusted connectivity is restored. Physical lock transitions, file protection, memory/latency and larger-scale acceptance remain open. Full reports/BQL/write/import/Widget parity and the default-mode switch remain parent-task work.
